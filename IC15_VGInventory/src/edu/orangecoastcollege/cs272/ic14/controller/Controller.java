...
public class Controller {
	...
	public static Controller getInstance() {
		if (theOne == null) {
			...
			try {
				...
				ArrayList<ArrayList<String>> rs = theOne.mUserDB.getAllRecords();
				...
				for (ArrayList<String> values : rs) {
					...
					/*
					 * cannot return a ResultSet, bc if you do the DBs need to stay open
					 * instead we are going to return an ArrayList of ArrayLists of Strings
					 */
	...
	public String signInUser(String email, String password) {
		for (User u : theOne.mAllUsersList)
			...
			{
				try {
					ArrayList<ArrayList<String>> rs = theOne.mUserDB.getRecord(String.valueOf(u.getId()));
					String storedPassword = rs.get(0).get(4);
	...
	public ObservableList<VideoGame> getGamesForCurrentUser()
	{
		...
		{
			try {
				ArrayList<ArrayList<String>> rs = theOne.mUserGamesDB.getRecord(String.valueOf(mCurrentUser.getId()));
